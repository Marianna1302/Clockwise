        ...
        if (value && ts.isArrayLiteralExpression(value)) {
            for (let i = 0; i < value.elements.length; i += 2) {
                ...
                if (loaderName === 'actions') {
                    const actionsForLogic = (_a = otherLogicType
                        .getProperties()) === null || _a === void 0 ? void 0 : _a.find((p) => p.getName() === 'actionCreators');
                    if (actionsForLogic &&
                        ...
                        for (const actionType of actionTypes || []) {
                            if (ts.isPropertySignature(actionType)) {
                                ...
                                if (lookup[name] && ts.isFunctionTypeNode(functionTypeNode)) {
                                    const parameters = functionTypeNode.parameters.map((param) => (0, utils_1.getParameterDeclaration)(param));
                                    ...
                                    if (ts.isTypeLiteralNode(returnType)) {
                                        ...
                                        if (ts.isPropertySignature(payload)) {
                                            const returnTypeNode = (0, ts_clone_node_1.cloneNode)(payload.type);
                                            (0, utils_1.gatherImports)(actionType, checker, parsedLogic);
                ...
                if (loaderName === 'values' || loaderName === 'props') {
                    const valuesForLogic = (_b = otherLogicType.getProperties()) === null || _b === void 0 ? void 0 : _b.find((p) => p.getName() === 'values');
                    if (valuesForLogic) {
