...
function visitListeners(type, inputProperty, parsedLogic) {
    ...
    if (ts.isObjectLiteralExpression(objectLiteral)) {
        extraActions = (0, utils_1.extractImportedActions)(objectLiteral, checker, parsedLogic);
        if (extraActions) {
            ...
            if (action) {
                parsedLogic.listeners.push({
                    ...
                    action: ts.factory.createTypeLiteralNode([
                        ts.factory.createPropertySignature(undefined, ts.factory.createIdentifier('type'), undefined, ts.factory.createLiteralTypeNode(ts.factory.createStringLiteral(getActionType(action.name)))),
                        ts.factory.createPropertySignature(undefined, ts.factory.createIdentifier('payload'), undefined, action.returnTypeNode),
                    ]),
                });
            }
        }
